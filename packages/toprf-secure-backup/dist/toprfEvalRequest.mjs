import { Some, kCombinations, lagrangeInterpolationForPoints, pubKeyToSec1, thresholdSame } from "@metamask/auth-network-utils";
import { generateJsonRPCObject } from "@toruslabs/http-helpers";
import { secp256k1 } from "ethereum-cryptography/secp256k1";
import { EXISTING_USER_AUTHENTICATION_THRESHOLD, JRPC_METHODS } from "./constants.mjs";
import { deriveAuthenticationKeyPair } from "./keyDerivation.mjs";
 ...
 */
export const createToprfEvalRequestParams = (authToken, blindedInputX, blindedInputY, verifier, verifierId) => {
    return {
        authToken,
        shareCoefficient: '1',
        blindedInputX,
 ...
 */
export const createToprfEvalRequest = async (endpoint, params) => {
    const toprfEvalJRPCRequest = generateJsonRPCObject(JRPC_METHODS.TOPRF_EVAL_REQUEST, params);
    return postJRPCRequest(endpoint, toprfEvalJRPCRequest);
};
/**
 ...
 */
export const evaluateSeed = async (hashedInput, randomScalar, resultArr) => {
    ...
    if (completedRequests.length >= EXISTING_USER_AUTHENTICATION_THRESHOLD) {
        const thresholdAuthPubKey = thresholdSame(completedRequests.map((res) => res.result?.pubKey), EXISTING_USER_AUTHENTICATION_THRESHOLD);
        if (thresholdAuthPubKey) {
            ...
            const allCombis = kCombinations(completedRequests.length, EXISTING_USER_AUTHENTICATION_THRESHOLD);
            let seed = null;
            for (const currentCombi of allCombis) {
                ...
                const curvePoints = currentCombiPoints.map((point) => secp256k1.ProjectivePoint.fromHex(`04${point.x}${point.y}`));
                ...
                const reconstructedPoint = lagrangeInterpolationForPoints(secp256k1.CURVE.n, curvePoints, nodeIndexes);
                // Unblind and hash the result
                const recoveredSeed = OPRF.unblindAndHash(hashedInput, reconstructedPoint, randomScalar);
...
export const recoverTOPRFSeed = async (params) => {
    ...
    const promiseArr = authTokens.map(async (authToken) => {
        ...
        const requestParams = createToprfEvalRequestParams(authToken.authToken, a.x.toString(16), a.y.toString(16), verifier, verifierId);
        return createToprfEvalRequest(endpoint, requestParams);
    });
    const result = await Some(promiseArr, async (resultArr) => evaluateSeed(userPasswordHash, r, resultArr));
    if (!result) {
        throw new Error('Insufficient toprf eval request results');
    }
    return result;
};
//# sourceMappingURL=toprfEvalRequest.mjs.map