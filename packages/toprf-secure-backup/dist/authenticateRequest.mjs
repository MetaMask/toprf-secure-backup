...
import { EXISTING_USER_AUTHENTICATION_THRESHOLD, JRPC_METHODS, NEW_USER_AUTHENTICATION_THRESHOLD } from "./constants.mjs";
...
const sendAuthenticateRequest = async (endpoint, params) => {
    ...
    return postJRPCRequest(endpoint, authJRPCRequest);
};
/**
 ...
 */
export const validateThresholdAuthenticateResponses = async (resultArr) => {
    ...
    if (completedRequests.length < EXISTING_USER_AUTHENTICATION_THRESHOLD) {
        throw TOPRFError.invalidAuthenticateResults(`Not enough completed requests. Expected: ${EXISTING_USER_AUTHENTICATION_THRESHOLD}, got: ${completedRequests.length}`);
    }
    ...
    const thresholdPubData = thresholdSame(pubData, EXISTING_USER_AUTHENTICATION_THRESHOLD);
    if (!thresholdPubData) {
        throw TOPRFError.invalidAuthenticateResults(`Threshold pubKey not found for ${JSON.stringify(pubData)}`);
    }
    const newUser = !thresholdPubData.pubKey;
    const hasMaxResponses = completedRequests.length >= NEW_USER_AUTHENTICATION_THRESHOLD;
    ...
    if (newUser && !hasMaxResponses) {
        throw TOPRFError.invalidAuthenticateResults(`Not enough completed requests. Expected: ${NEW_USER_AUTHENTICATION_THRESHOLD}, got: ${completedRequests.length}`);
    }
...
export const authenticateUser = async (params) => {
    ...
    const results = await Some(promiseArr, async (responses) => validateThresholdAuthenticateResponses(responses));
