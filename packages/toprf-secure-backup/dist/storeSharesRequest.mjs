import { getProxyCoordinatorNodeIndex, pubKeyToSec1, toSnakeCaseKeys } from "@metamask/auth-network-utils";
...
export const createStoreKeySharesRequestParams = async (params) => {
    ...
    const shareImportItems = await generateShareImportItems(nodeEndpointsMap, authTokens, oprfKey, keyIndex);
    return {
 ...
 */
export const sendStoreKeySharesRequest = async (endpoint, params) => {
 ...
 * @returns The store key shares request promise.
 */
export const storeKeyShares = async (params) => {
    ...
    const proxyNodeEndpointIndex = getProxyCoordinatorNodeIndex(authTokens.map((token) => token.nodeIndex), verifier, verifierId);
    const proxyNodeEndpoint = nodeEndpointsMap[proxyNodeEndpointIndex];
    const storeReqPromise = await sendStoreKeySharesRequest(proxyNodeEndpoint, requestParams);
