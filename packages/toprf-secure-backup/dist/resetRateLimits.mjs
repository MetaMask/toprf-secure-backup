 ...
 */
export const createResetRateLimitRequestParams = (authToken, signature, signedData, verifier, verifierId) => {
    return {
        authToken,
        signature,
        signedData,
 ...
 */
export const sendResetRateLimitRequest = async (endpoint, params) => {
    const resetRateLimitJRPCRequest = generateJsonRPCObject(JRPC_METHODS.RESET_RATE_LIMIT_REQUEST, params);
    return postJRPCRequest(endpoint, resetRateLimitJRPCRequest);
};
/**
 ...
 */
export const validateThresholdResetRateLimitResponses = async (resultArr, threshold) => {
    ...
    if (completedRequests.length < threshold) {
        throw TOPRFError.invalidAuthenticateResults(`invalid reset rate limit results, expected ${threshold} but got ${completedRequests.length}`);
    }
...
export const resetRateLimits = async (params) => {
    ...
    const promiseArr = authTokens.map(async (authToken) => {
        ...
        const requestParams = createResetRateLimitRequestParams(authToken.authToken, signature, signedData, verifier, verifierId);
        return sendResetRateLimitRequest(endpoint, requestParams);
    });
    const result = await Some(promiseArr, async (resultArr) => validateThresholdResetRateLimitResponses(resultArr, authTokens.length));
    if (!result) {
        throw new Error('Insufficient reset rate limit request results');
    }
};
//# sourceMappingURL=resetRateLimits.mjs.map