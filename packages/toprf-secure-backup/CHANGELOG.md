...
- Add optional `client` constructor parameter to `ToprfSecureBackup`. When set, SSS JSON-RPC requests include an `x-web3-client` header (name and version, e.g. `metamask-extension@13.46.1`) for SSS service logging. The header is omitted when `client` is missing or empty. ([#162](https://github.com/MetaMask/toprf-secure-backup/pull/162))
