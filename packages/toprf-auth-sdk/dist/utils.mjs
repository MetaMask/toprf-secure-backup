...
import { encParamsHexToBuf, encryptedParamsBufToHex, generateRandomPolynomial, getSecp256K1Curve, toCamelCaseKeys, toSnakeCaseKeys } from "@metamask/auth-network-utils";
import { decrypt, encrypt } from "@toruslabs/eccrypto";
import { post } from "@toruslabs/http-helpers";
import $BN from "bn.js";
const BN = $importDefault($BN);
import { NODE_URLS } from "./constants.mjs";
/**
 ...
 */
export const generateShareImportItems = async (nodeIndexes, nodePubkeys, authTokens, privKey, keyIndex) => {
    ...
    const encryptionPromises = nodeIndexes.map(async (nodeIndex, i) => {
        ...
        const shareJson = shares[new BN(nodeIndex).toString('hex', 64)].toJSON();
 ...
 */
export const postJRPCRequest = async (endpoint, request) => {
    const req = { ...request };
    const params = toSnakeCaseKeys(request.params);
    req.params = params;
    return post(endpoint, req, {}, { logTracingHeader: false }).then((res) => {
        if (res.result) {
 ...
 */
export const decryptAuthToken = async (authToken, sessionPrivateKey) => {
    const ecCurve = getSecp256K1Curve();
    ...
    const decryptedAuthToken = await decrypt(decryptionKey.getPrivate().toArrayLike(Buffer), {
        ...metadata,
