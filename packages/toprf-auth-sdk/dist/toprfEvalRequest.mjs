...
export const createToprfEvalRequest = async (endpoint, params) => {
     ...
     */
    const toprfEvalResponse = postJRPCRequest(endpoint, toprfEvalJRPCRequest);
 ...
 */
export const evaluateSeed = async (hashedInput, randomScalar, resultArr, threshold) => {
    ...
    if (completedRequests.length >= threshold) {
        const thresholdAuthPubKey = thresholdSame(completedRequests.map((res) => res.result?.pubKey), threshold);
...
export const recoverTOPRFSeed = async (params) => {
    ...
    const result = await Some(promiseArr, async (resultArr) => evaluateSeed(hashedInput, r, resultArr, 3));
    if (!result) {
