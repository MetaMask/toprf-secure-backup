...
describe("common utils", function () {
  it("should be able to hash a buffer and hexify the result with `keccak256AndHexify", function () {
    ...
    const expectedHashedHexString = Buffer.from(keccak256(buffer)).toString("hex");

    const hexString = keccak256AndHexify(buffer);
    expect(hexString).toEqual("0x9c22ff5f21f0b81b113e63f7db6da94fedef11b2119b4088b89664fb9a3cb658");
    expect(expectedHashedHexString).toEqual(hexString.slice(2));
  });
  ...
  describe("`Some` function", function () {
    ...
    const callbackFnFactory = (threshold: number) => async (resultArr: { data: string }[]) => {
      const completedResult = resultArr.filter((result) => !!result?.data);
    ...
    it("`Some` and `SomeV1` should return the same result", async function () {
      ...
      expect(result).toEqual(genericResult);
    });
    ...
    it("`Some` should throw an error when not enough promises are resolved", async function () {
      ...
      expect(() => Some(promises, callbackFn)).rejects.toThrow("not enough data");
    });
  });
});
