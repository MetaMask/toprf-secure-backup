...
class Polynomial {
    ...
    constructor(polynomial, ecCurve, nobleOptions) {
        ...
        if (nobleOptions) {
            ...
            __classPrivateFieldSet(this, _Polynomial_fieldOps, Field(nobleOptions.curveN), "f");
            // Convert bigint to BN for backward compatibility
            this.polynomial = __classPrivateFieldGet(this, _Polynomial_coefficients, "f").map((coefficient) => new BN(coefficient.toString()));
        }
        else {
    ...
    generateShares(shareIndexes) {
        ...
        for (const index of newShareIndexes) {
            shares[index.toString('hex', 64)] = new Share(index, this.polyEval(index));
        }
        return shares;
    }
}
...
//# sourceMappingURL=polynomial.mjs.map