...
import { generate32BytesPrivateKeyBuffer } from "./cryptoUtils.mjs";
import { generateEmptyBNArray } from "./helpers.mjs";
import Point from "./point.mjs";
import Polynomial, { PolynomialNoble } from "./polynomial.mjs";
 ...
 */
export function lagrangeInterpolatePolynomial(ecCurve, points) {
 ...
 */
export function generateRandomPolynomial(ecCurve, degree, secret, deterministicShares) {
    let actualS;
    if (secret) {
        actualS = secret;
    }
    ...
    for (let i = 0; i < degree - deterministicShares.length; i += 1) {
        ...
        points[shareIndex.toString('hex', 64)] = new Point(shareIndex, new BN(generate32BytesPrivateKeyBuffer(ecCurve)), ecCurve);
    }
 ...
 * @returns The polynomial
 */
export function generateRandomPolynomialNoble(curveN, degree, secret) {
 ...
 */
export function generateRandomScalar(curveN = secp256k1.CURVE.n) {
 ...
 */
export function lagrangeInterpolationNoble(curveN, values, nodeIndexes) {
    ...
    let result = (isScalar ? 0n : secp256k1.ProjectivePoint.ZERO);
 ...
 */
export function lagrangeInterpolationForPoints(curveN, points, nodeIndexes) {
 ...
 */
export function lagrangeInterpolationForScalars(curveN, scalars, nodeIndexes) {
 ...
 */
export function lagrangeInterpolation(ecCurve, shares, nodeIndex) {
    if (shares.length !== nodeIndex.length) {
        throw new Error('shares not equal to nodeIndex length in lagrangeInterpolation');
    }
    // Convert shares and indices to points
    const points = shares.map((share, i) => new Point(nodeIndex[i], share, ecCurve));
...
//# sourceMappingURL=lagrangeInterpolation.mjs.map